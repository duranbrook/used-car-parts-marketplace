            ...
            <div className="grid grid-cols-2 gap-3">
              {(["BUYER", "SELLER"] as const).map((role) => (
                <button
                  key={role}
                  onClick={() => {
                    document.cookie = `oauth-intended-role=${role}; path=/; max-age=300; SameSite=Lax`;
                    signIn("google", { callbackUrl: "/dashboard" });
                  }}
                  className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg border border-gray-300 bg-white text-gray-700 font-medium hover:bg-gray-50 text-sm"
                >
